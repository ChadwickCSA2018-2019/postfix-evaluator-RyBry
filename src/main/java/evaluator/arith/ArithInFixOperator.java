...
public class ArithInFixOperator implements PostFixEvaluator<Integer> {
  ...
  /**
   * Method evaluates an InFix expression.
   * @return the result NOTE: This does not work with the negate expression due to
   *         it's Postfix intrinsic nature. Multiplying by -1 will work though!
   */
  @Override
  public Integer evaluate(String expr) {
    ...
    while (parser.hasNext()) {
      ...
      case OPERAND:
        ...
        if (!parser.hasNext()) {
          // If there is only one element left in the stack, return it before end of loop
          // to prevent popping from an empty stack.
          if (operandStack.size() == 1) {
            return operandStack.pop().getValue();
          } else if (operandStack.size() > 1 && !parser.hasNext() && operatorStack.isEmpty()) {
            // If there is more than 1 element left with no other operations, the expression
            // did not evaluate properly
      ...
      case OPERATOR:
        ...
        // If the current operation takes higher priority than the previous, put it on
        // the stack
        if (operatorStack.isEmpty() || scannedOperator.getImportance() > operatorStack.top().getImportance()) {
          ...
          result = operation.performOperation();
          operandStack.push(result);
          // Then push the other operation onto the stack so it isn't lost in computer
          // space :D
          operatorStack.push(scannedOperator);
        }
        break;
